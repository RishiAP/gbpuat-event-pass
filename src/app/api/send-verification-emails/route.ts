...
const CONFIG = {
  ...
  CHUNK_DELAY: 6000,            // 6 second delay between chunks of 5 emails
  BATCH_DELAY: 2000,            // 2 second delay between batches
  RETRY_ATTEMPTS: 2,            // Retry failed emails
  RETRY_DELAY: 1000             // Delay before retry
};
// With this config: 5 emails take ~2.5s + 6s delay = 8.5s per chunk
// That's 5 emails per 8.5s = ~35 emails/minute ✅
